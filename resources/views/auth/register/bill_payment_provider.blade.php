...
<head>
    ...
    <title>@appName Bill Payment Provider Registration</title>
  ...
  <body>
    <div class="container">
      ...
      <div class="">
        ...
        <form class="form-signin needs-validation"
              id="regForm"
              action="{{ route('bpa_register') }}"
              method="POST">

          <div class="form-group" id="textDescribe">
            <h3 class="" style="margin: 30px;"><center>Make your cutomers happy</center></h3>
          </div>
          ...
          <!-- Bill payment Type -->
            <div class="row">
                ...
                <div class="form-group col-12 col-md-12">
                        <select class="form-control"
                                id="type_of_bill_payment"
                                name="type_of_bill_payment"
                                onblur=" validate(event, 'required|integer') "
                                oninput=" validate(event, 'required|integer') "
                                required
                                >
                                <!--fetch from the database-->
                        <option  value="">Select The Type of Bill Payment</option>
                        @foreach( $bill_payment_types as $type )
                        @if( $type->id == old('type_of_bill_payment') )
                          <option value="{{ $type->id }}" selected>{{ $type->bill_payment_name }} Bill Payment</option>
                        @else
                          <option value="{{ $type->id }}">{{ $type->bill_payment_name }} Bill Payment</option>
                        @endif
                        @endforeach

                        </select>
                        <div class="form-control" 
                        id="type_of_bill_paymentError" 
                        style="display: none;">
            ...
            <div class="row">
                <div class="form-group col-md-12">
                  <input type="text"
                        ...
                        placeholder="Company Name,eg.Addis Ababa Water Provider"
                        name="company_name"
    ...
    <script src="{{ asset('/js/tabHandler_bpa.js') }}" type="text/javascript"></script>
